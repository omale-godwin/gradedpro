...
import INDUSTRIALCHEMISTRYModel from "../../components/models/INDUSTRIALCHEMISTRY";
...
export default async function (req, res) {
  ...
  if (method === "POST") {
    try {
      let business = await INDUSTRIALCHEMISTRYModel.create(req.body);
      res.status(200).json({
  ...
  if (method === "GET") {
    try {
      let bussines = await INDUSTRIALCHEMISTRYModel.find({
        id: { $gt: 10 * (page - 1) },
      }).limit(20);
