...
import ENVIRONMENTALModel from "../../components/models/ENVIRONMENTAL";
...
export default async function (req, res) {
  ...
  if (method === "POST") {
    try {
      let business = await ENVIRONMENTALModel.create(req.body);
      res.status(200).json({
  ...
  if (method === "GET") {
    try {
      let bussines = await ENVIRONMENTALModel.find({
        id: { $gt: 10 * (page - 1) },
      }).limit(20);
