...
import INDUSTRIALRELATIONSModel from "../../components/models/INDUSTRIALRELATIONS";
...
export default async function (req, res) {
  ...
  if (method === "POST") {
    try {
      let business = await INDUSTRIALRELATIONSModel.create(req.body);
      res.status(200).json({
  ...
  if (method === "GET") {
    try {
      let bussines = await INDUSTRIALRELATIONSModel.find({
        id: { $gt: 10 * (page - 1) },
      }).limit(20);
