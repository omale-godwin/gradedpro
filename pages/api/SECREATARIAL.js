...
import SECREATARIALModel from "../../components/models/SECREATARIAL";
...
export default async function (req, res) {
  ...
  if (method === "POST") {
    try {
      let business = await SECREATARIALModel.create(req.body);
      res.status(200).json({
  ...
  if (method === "GET") {
    try {
      let bussines = await SECREATARIALModel.find({
        id: { $gt: 10 * (page - 1) },
      }).limit(20);
